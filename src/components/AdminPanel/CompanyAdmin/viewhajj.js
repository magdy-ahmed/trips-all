...
export const AllTripsHajj = (props) => {
...
datatrips['search'] = searchtrips
    ...
    fetch('/api/search/trips/admin', {
      ...
      resp.json().then(data => {
        if (resp.ok &&data){
          setTrips({trips: data.trips.filter(trip => ((trip.type =='hajj')||(trip.type =='umrah' ))), limit: 10})
        }
      })
    });
  }, []);
        ...
        {trips_patch ? (trips_patch.map((trip, index) => {
          ...
          return(
            <tr className="table-light" key={index}>
              ...
              <td className="info-weight">{trip.duration?trip.duration+' يوم ':''}</td>
              ...
              <td className="info-weight">{trip.urlDetails}</td>
  ...
  const handleType = (e) => {
    ...
    setUpdateState({
      ...
      tid: updateState.tid,
      transport: updateState.transport,
      day: updateState.from,
      duration: updateState.duration,
      ...
      urlDetails: updateState.urlDetails
    })
    // Clear Forms
      ...
      <div>
          <div className="form-inline">
            ...
            <div className="input-group ">
              ...
              <div className="input-group">
              <label htmlFor="duration" className="mt-3 font-weight-bold">المدة بالأيام: </label>
                <input onChange={handleEditChange} type="number" onFocus={(e) => {e.target.type='duration'}} value={ updateState.duration } id="duration" placeholder="* المدة" className="form-control"/>
              </div>
              <div className="input-group">
                <select required onChange={handleEditChange} defaultValue={updateState.type} id="transport" className="custom-select">
              ...
              <div className="input-group">
                <label htmlFor="urlDetails" className="mt-3 font-weight-bold">رابط التفاصيل: </label>

                <input onChange={handleEditChange} type="text" onFocus={(e) => {e.target.type='urlDetails'}} value={ updateState.urlDetails } id="urlDetails" placeholder="* رابط التفاصيل" className="form-control"/>
              </div>
          </div>
        ...
        <div className="form-inline">
          ...
          <div className="input-group">
            <select required onChange={handleEditChange} defaultValue={updateState.from} id="from" className="custom-select">
              <option value="">مدينة الصعود</option>
              {fromCities ? (fromCities.map((city, index) => {
          ...
          <div className="input-group pt-special mr-md-2">
            <select required onChange={handleEditChange} defaultValue={updateState.to} id="to" className="custom-select">
              <option value="">مدينة الوصول</option>
              {ToCities ? (ToCities.map((city, index) => {
                return(<option value={city} key={index}>{city}</option>)
              })) : null}
            </select>
          </div>
    ...
    <div className="mb-4 pb-4 ">
      {show ? (
        <div className="container text-center mt-5 pt-5">
         ...
         <span className="display-7 mb-2 mr-3  "  aria-current="page">البرامج </span>
            ...
            <div className="table-responsive">
              ...
              <thead>
                <tr className="table-secondary">
                  ...
                  <th scope="col">مدينة الهبوط</th>
                  <th scope="col">وسيلة الأنتقال</th>
      ...
      <Modal show={isUpdate} onHide={resetUpdate}>
        ...
        <div>
          ...
          <div className="container text-right mr-md-4">
            <p className="font-weight-bold text-primary">تعديل البرنامج رقم {updateState.tid}# : </p>
          </div>

          <div className="mt-3 text-right">
            <label htmlFor="type" className="mt-3 font-weight-bold">نوع البرنامج: </label>
            <div className="form-inline">
              <div className="input-group">
