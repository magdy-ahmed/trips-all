...
export const EditOffer = (props) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/orders/${offerInfo.transOID}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
      ...
      resp.json().then(data => {
        if (resp.ok){
          ...
          setState({
            ...
            price_per_installment: offerInfo.info.installment_price,
            price_per_document: (parseInt(offerInfo.price)/parseInt(data.order.num_documents)).toString()
  ...
  const handleSubmit = (e) => {
    ...
    const submitOffer = async() => {
      await fetch(`/api/transactions/offer/${offerInfo.oid}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json'
      ...
      if (order.type === 'IESY'){
        return(
          <div className="input-group">
            {company === "All" ? (
              <select defaultValue={offerInfo.company} id="company" onChange={handleChange} className="custom-select">
      ...
      return(
        <form onSubmit={handleSubmit} name="offerForm" className="container text-center mt-4">
          ...
          <div className="form-inline justify-content-center mt-4">
            ...
            <div className="input-group">
              <input defaultValue={parseInt(parseFloat(offerInfo.price) / parseFloat(order.num_documents))} onChange={handleChange} type="text" required id="price_per_document" placeholder="السعر لكل وثيقة (جواز/هوية(" className="form-control"/>
            </div>
            ...
            <div className="input-group pt-special">
              <input disabled type="text" id="price" value={`الإجمالي : ${(parseInt(order.num_documents)*parseInt(state.price_per_document))||parseInt(offerInfo.price)}`} className="form-control"/>
            </div>
          ...
          <div className="input-group justify-content-center mt-4">
            <button type="submit" id="SubmissionBtn" className="btn btn-md btn-outline-grey">تعديل العرض</button>
          </div>
      ...
      return(
        <form onSubmit={handleSubmit} name="offerForm" className="container text-center mt-4">
          ...
          <div className="form-inline justify-content-center mt-4">
            ...
            <div className="input-group">
              <input defaultValue={parseInt(parseFloat(offerInfo.price)/parseFloat(offerInfo.info.num_installments))} onChange={handleChange} required id="price_per_installment" type="text" placeholder="السعر لكل قسط" className="form-control"/>
            </div>
            ...
            <label htmlFor="price" className="ml-sm-4"/>
            <div className="input-group pt-special">
              <input disabled type="text" id="price" value={`الإجمالي : ${parseInt(getInstallments(order.service) * state.price_per_installment)||parseInt(offerInfo.price)}`} className="form-control"/>
            </div>
